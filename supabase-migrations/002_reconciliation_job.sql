...
CREATE OR REPLACE FUNCTION reconcile_multiplications_daily_metrics(target_date DATE)
RETURNS VOID AS $$
DECLARE
  time_buckets JSONB;
...
CREATE OR REPLACE FUNCTION reconcile_multiplications_previous_day()
RETURNS VOID AS $$
DECLARE
  yesterday DATE;
BEGIN
  yesterday := (CURRENT_DATE AT TIME ZONE 'America/New_York')::date - INTERVAL '1 day';
  PERFORM reconcile_multiplications_daily_metrics(yesterday);
END;
$$ LANGUAGE plpgsql;
...
--   'reconcile-multiplications-daily-metrics',
--   '0 2 * * *', -- 2 AM daily
--   'SELECT reconcile_multiplications_previous_day();'
-- );

-- Manual function to run reconciliation for a date range
CREATE OR REPLACE FUNCTION reconcile_multiplications_date_range(start_date DATE, end_date DATE)
RETURNS VOID AS $$
DECLARE
  current_date DATE;
BEGIN
  ...
  WHILE current_date <= end_date LOOP
    PERFORM reconcile_multiplications_daily_metrics(current_date);
